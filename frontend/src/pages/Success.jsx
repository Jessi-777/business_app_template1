...
export default function Success() {
  ...
  // ✅ Bug 3 Fixed: fetch order by stripeSessionId, not just "most recent"
  const fetchOrderDetails = async () => {
    try {
      // First try: look up by stripeSessionId (most accurate)
      const response = await fetch(`${API_URL}/api/orders/session/${sessionId}`);
      if (response.ok) {
        const order = await response.json();
        setOrderDetails(order);
      } else {
        // Fallback: get most recent order (only if session endpoint not yet added)
        const fallback = await fetch(`${API_URL}/api/orders?limit=1`);
        if (fallback.ok) {
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden" id="main-content">
                ...
                <div className="mt-4 p-3 bg-indigo-900/30 border border-indigo-500/50 rounded-lg">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-indigo-300">
                      Affiliate code <code className="font-mono font-bold">{orderDetails.affiliate.code}</code> applied — athlete gets credited! 🏆
                    </span>
                  </div>
                </div>
              )}
      ...
      <style jsx>{`
        ...
        .animate-draw-check { animation: draw-check 0.5s ease-in-out 0.3s forwards; }
